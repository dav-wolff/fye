use std::{convert::Infallible, future::{self, Future}, marker::{PhantomData, Send}, ops::{Deref, DerefMut}, path::Path, pin::Pin, sync::Arc};
...
impl ManageConnection for ConnectionManager {
	...
	fn connect(&self) -> Result<Self::Connection, Self::Error> {
		db::establish_connection(&self.url)
	}
...
impl<'a> DbConnection<'a> {
	fn new(conn: PooledConnection<ConnectionManager>) -> Self {
		Self(ConnectionKind::Pooled(conn, PhantomData::default()))
	}
